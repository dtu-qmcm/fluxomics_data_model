...
class FluxMLWriter:
    ...
    def _add_simulation(
        ...
        # Auto-derive starting values for drain reactions not already listed.
        # Drain reactions ({metab}_out) are generated by _add_reaction_network
        # but are absent from the freeflux simulation data. x3cflux crashes when
        # their fluxes default to zero, so we solve S*v=0 for them here.
        ...
        sink_mets = sorted(produced - consumed - input_pools)
        drain_ids = [f"{m}_out" for m in sink_mets]
        missing_drains = [d for d in drain_ids if d not in stored_ids]
        ...
        if missing_drains:
            ...
            stored_vals = {fv.flux: (fv.value or 0.0) for fv in experiment.simulation.variables.flux_values}
            ...
            if d_cols:
                ...
                try:
                    ...
                    drain_vals = {d_names[i]: float(v_d[i]) for i in range(len(d_cols))}
                except Exception:
                    drain_vals = {n: 0.0 for n in d_names}

                for drain_id in missing_drains:
                    fv_elem = ET.SubElement(variables, "fluxvalue")
                    fv_elem.set("flux", drain_id)
                    fv_elem.set("type", "net")
                    fv_elem.text = str(drain_vals.get(drain_id, 0.0))
